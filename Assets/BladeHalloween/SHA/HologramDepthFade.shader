...
Shader "SciFi/HologramDepthFade"
{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float2 panner13 = ( 0.7 * _Time.y * appendResult24 + ( appendResult10 * _UV ));
			...
			#endif
			float4 temp_output_18_0 = ( tex2Dlod( _Hologram, float4( panner13, 0, 0.5) ) * staticSwitch25 );
			v.vertex.y -= ( _VertexMovement * temp_output_18_0 ).g;
		}
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 appendResult10 = (float2(ase_worldPos.x , ase_worldPos.y));
			float2 panner13 = ( 0.5 * -_Time.y * appendResult24 + ( appendResult10 * _UV ));
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			...
			float distanceDepth32 = abs( ( screenDepth32 - LinearEyeDepth( ase_screenPosNorm.x ) ) / ( _DepthPower ) );
